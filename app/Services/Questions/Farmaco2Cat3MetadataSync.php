...
final class Farmaco2Cat3MetadataSync
{
    /**
     ...
     */
    public static function sync(int $materiaId = Farmaco2Cat3SectionCatalog::MATERIA_ID): array
    {
        $lista = QuestionBankLocator::loadCanonicalList($materiaId);
        if ($lista === []) {
            throw new \RuntimeException(
                'Banco JSON vazio para matéria '.$materiaId.'. Execute: php artisan questions:import-farmaco2cat3'
            );
        }

        $catedraId = Farmaco2Cat3CatalogInstaller::catedraId();
        $inserted = 0;
        $updated = 0;
        $semSeccion = 0;
        $porParcial = [];
        ...
        foreach ($lista as $overlayKey => $blob) {
            ...
            $resolved = Farmaco2Cat3SectionCatalog::resolve($seccion);
            ...
            $attrs = [
                ...
                'parcial' => $parcial,
                'tema' => $tema,
                'fonte' => Farmaco2Cat3SectionCatalog::FONTE,
                'is_demo' => false,
            ];
